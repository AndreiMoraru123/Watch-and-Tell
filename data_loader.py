...
def get_loader(transform, mode='train', batch_size=1, vocab_threshold=None, vocab_file='./vocab.pkl',
    ...
    if mode == 'train':
        if vocab_from_file:
            assert os.path.exists(vocab_file), "vocab_file does not exist.  Change vocab_from_file to False to create " \
                                               "vocab_file. "
...
class CocoDataset(data.Dataset):
    ...
    def __init__(self, transform, mode, batch_size, vocab_threshold, vocab_file, start_word, end_word, unk_word,
                 vocab_from_file, img_folder, annotations_file):
        """
        Args:
          transform: Image transform.
          :param mode: One of 'train' or 'test'.
          ...
          annotations_file: COCO format annotation file for images.
        """
